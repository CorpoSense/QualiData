...
@router.post("/api/datasets/{dataset_id}/operations/fix-case", response_model=OperationResponse)
async def fix_case(
    dataset_id: int,
    column: str,
    ...
    save_operation(dataset_id, "fix_case", {"column": column, "case": case}, before, after, session)
    await session.commit()

    return OperationResponse(status="success", message=f"Fixed case to '{case}' in '{column}'", columns=dataset.columns)


@router.post("/api/datasets/{dataset_id}/operations/trim-whitespace", response_model=OperationResponse)
async def trim_whitespace(
    ...
    save_operation(dataset_id, "trim_whitespace", {"columns": cols}, before, after, session)
    await session.commit()

    return OperationResponse(status="success", message=f"Trimmed whitespace from {len(trimmed_cols)} columns", columns=dataset.columns)


@router.post("/api/datasets/{dataset_id}/operations/fix-typos", response_model=OperationResponse)
async def fix_typos(
    dataset_id: int,
    column: str,
    mappings: dict,
    ...
    save_operation(dataset_id, "fix_typos", {"column": column, "mappings": mappings}, before, after, session)
    await session.commit()

    return OperationResponse(status="success", message=f"Fixed typos in '{column}'", columns=dataset.columns)


@router.post("/api/datasets/{dataset_id}/operations/standardize-values", response_model=OperationResponse)
async def standardize_values(
    ...
    mappings = {
        'yes': 'Yes', 'y': 'Yes', 'Y': 'Yes', 'YES': 'Yes', 'true': 'Yes', 'True': 'Yes', 'TRUE': 'Yes', '1': 'Yes',
        'no': 'No', 'n': 'No', 'N': 'No', 'NO': 'No', 'false': 'No', 'False': 'No', 'FALSE': 'No', '0': 'No',
    ...
    save_operation(dataset_id, "standardize_values", {"column": column}, before, after, session)
    await session.commit()

    return OperationResponse(status="success", message=f"Standardized values in '{column}'", columns=dataset.columns)
