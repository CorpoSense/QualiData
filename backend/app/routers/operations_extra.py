...
# Routes
@router.post("/datasets/{dataset_id}/operations/filter-rows", response_model=OperationResponse)
async def filter_rows(dataset_id: int, request: FilterRowsRequest, current_user: User = Depends(get_current_active_user), session: AsyncSession = Depends(get_async_session)):
...
@router.post("/datasets/{dataset_id}/operations/sort-data", response_model=OperationResponse)
async def sort_data(dataset_id: int, request: SortDataRequest, current_user: User = Depends(get_current_active_user), session: AsyncSession = Depends(get_async_session)):
    dataset = get_dataset_with_owner_check(dataset_id, current_user.id, session)
    if not dataset.preview_data:
        raise HTTPException(status_code=400, detail="No data to operate on")
...
@router.post("/datasets/{dataset_id}/operations/remove-duplicates", response_model=OperationResponse)
async def remove_duplicates(dataset_id: int, request: RemoveDuplicatesRequest, current_user: User = Depends(get_current_active_user), session: AsyncSession = Depends(get_async_session)):
    dataset = get_dataset_with_owner_check(dataset_id, current_user.id, session)
    if not dataset.preview_data:
        raise HTTPException(status_code=400, detail="No data to operate on")
...
@router.post("/datasets/{dataset_id}/operations/find-replace", response_model=OperationResponse)
async def find_replace(dataset_id: int, request: FindReplaceRequest, current_user: User = Depends(get_current_active_user), session: AsyncSession = Depends(get_async_session)):
    dataset = get_dataset_with_owner_check(dataset_id, current_user.id, session)
    if not dataset.preview_data:
        raise HTTPException(status_code=400, detail="No data to operate on")
...
@router.post("/datasets/{dataset_id}/operations/change-type", response_model=OperationResponse)
async def change_type(dataset_id: int, request: ChangeTypeRequest, current_user: User = Depends(get_current_active_user), session: AsyncSession = Depends(get_async_session)):
    dataset = get_dataset_with_owner_check(dataset_id, current_user.id, session)
    if not dataset.preview_data:
        raise HTTPException(status_code=400, detail="No data to operate on")
